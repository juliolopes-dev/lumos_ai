...
import { CreateAssistantData } from '../types';

interface CreateAssistantModalProps {
  isOpen: boolean;
  onClose: () => void;
  onCreate: (data: CreateAssistantData) => void;
}
...
const CreateAssistantModal: React.FC<CreateAssistantModalProps> = ({ isOpen, onClose, onCreate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim() && systemInstruction.trim()) {
      onCreate({ name, systemInstruction, temperature });
      setName('');
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg transition-colors shadow-lg shadow-emerald-900/20"
            >
              Criar Assistente
            </button>
          </div>
